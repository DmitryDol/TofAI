...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, beta, x_norm, mu, var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x, gamma, beta, x_norm, mu, var, eps = cache
    N = dout.shape[0]
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    m = np.max(window)
                    dx[n, c, h_start:h_end, w_start:w_end] += (window == m) * dout[
                        n, c, i, j
                    ]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_norm_flat = (x_reshaped - mu) / np.sqrt(var + eps)
    x_norm = x_norm_flat.reshape(N, C, H, W)

    out = x_norm * gamma[None, :, None, None] + beta[None, :, None, None]
    cache = (x, x_norm_flat, mu, var, gamma, beta, G, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, x_norm_flat, mu, var, gamma, beta, G, eps = cache
    N, C, H, W = x.shape

    dbeta = np.sum(dout, axis=(0, 2, 3))
    dgamma = np.sum(dout * x_norm_flat.reshape(N, C, H, W), axis=(0, 2, 3))
    ...
    dx_reshaped = (
        ...
        * (1.0 / np.sqrt(var + eps))
        * (
            D_group * dx_norm_reshaped
            - np.sum(dx_norm_reshaped, axis=1, keepdims=True)
            - x_norm_flat * np.sum(dx_norm_reshaped * x_norm_flat, axis=1, keepdims=True)
        )
    )
