...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out_aff1, cache_aff1 = affine_forward(X, W1, b1)
        out_relu1, cache_relu1 = relu_forward(out_aff1)
        scores, cache_aff2 = affine_forward(out_relu1, W2, b2)
        ...
        dout_aff2, dW2, db2 = affine_backward(dscores, cache_aff2)
        dW2 += self.reg * W2

        dout_relu1 = relu_backward(dout_aff2, cache_relu1)

        dx, dW1, db1 = affine_backward(dout_relu1, cache_aff1)
        dW1 += self.reg * W1
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params[f"W{i + 1}"] = np.random.normal(
                0.0, weight_scale, (layer_dims[i], layer_dims[i + 1])
            )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_dropout:
                ...
                caches[f"drop{i}"] = cache_drop

        W_out = self.params[f"W{self.num_layers}"]
        b_out = self.params[f"b{self.num_layers}"]
        scores, cache_aff_out = affine_forward(out, W_out, b_out)
        caches[f"aff{self.num_layers}"] = cache_aff_out
        ...
        dout, dW, db = affine_backward(dscores, caches[f"aff{self.num_layers}"])
